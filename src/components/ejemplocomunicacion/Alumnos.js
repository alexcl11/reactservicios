      ...
      <tbody>
        {
            this.state.alumnos.map((alumno, index) => {
                return(<tr key={index}><td>{alumno.idAlumno}</td><td>{alumno.nombre}</td><td>{alumno.apellidos}</td><td><img src={alumno.imagen} style={{"width":"50px"}}/></td><td><button onClick={() => this.mandarAlumnoDetalles(alumno)} className='btn btn-primary'>Detalles</button></td></tr>)
            })
        }
      </tbody>
    )
